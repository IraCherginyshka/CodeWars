...
function getMissingIngredients(recipe, added) {
  ...
  for (let key in added) {
    if (recipe[key] < added[key]) {
      let n = added[key] / recipe[key];
      if (n > amount) {
        amount = Math.ceil(n);
      }
    }
  ...
  for (let key in recipe) {
    if (key in added) {
      missingIngredients[key] = recipe[key] * amount - added[key];
    } else {
      missingIngredients[key] = recipe[key] * amount;
    }
  }

  for (let key in missingIngredients) {
    if (missingIngredients[key] === 0) {
...
let recipe1 = {flour: 200, eggs: 1, sugar: 100};

console.log(getMissingIngredients(recipe1, {flour: 50, eggs: 1})); // must return {flour: 150, sugar: 100}
console.log(getMissingIngredients(recipe1, {sugar: 100})); // must return {flour: 200, eggs: 1, sugar: 100}
console.log(getMissingIngredients(recipe1, {flour: 500, sugar: 200})); // must return {flour: 100, eggs: 3, sugar: 100}
